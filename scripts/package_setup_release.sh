...
#
# Writes: dist/teambuddies-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix teambuddies \
  --exe-name Team_Buddies \
  --omit-openbios \
  --display-name "Team Buddies" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Team Buddies (Europe) disc" \
  --runtime-dir mods \
